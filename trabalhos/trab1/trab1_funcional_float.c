...
int pushNumStck (tStack* ptr, float x); //pushes a number into a node into the stack

int pushCharStck (tStack* ptr, char c);

float popNumStck (tStack* ptr); //pops a number out of a stack (and deletes it)
...
int operate(tStack* numStckPtr, tStack* opStckPtr){
	...
	float n1 = popNumStck(numStckPtr), n2 = popNumStck(numStckPtr);
...
void printStcks (tStack* numStckPtr, tStack* opStckPtr){
	...
	if(numStckPtr->top != NULL && opStckPtr->top != NULL) printf("Ultimo PushNum: %f\nUltimo PushChar: %c\n", numStckPtr->top->x, opStckPtr->top->c);
